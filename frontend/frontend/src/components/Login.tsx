...
const Login: React.FC = () => {
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex' }}>
      ...
      {/* Right: Login form centered, with back button */}
      <Box
        sx={{
          width: { xs: '100%', md: '50%' },
          display: 'flex',
              ...
              <form onSubmit={handleSubmit}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 2, md: 3 } }}>
                  <TextField
                    ...
                    required
                    error={Boolean(error)}
                    helperText={error && <span style={{ color: 'rgba(255,77,79,0.7)', fontSize: 12, lineHeight: '18px', fontFamily: 'Poppins, sans-serif', textAlign: 'left' }}>Incorrect email or password</span>}
                    sx={{
                      background: 'white',
                      borderRadius: 0.5,
                  ...
                  <Button
                    ...
                    fullWidth
                    startIcon={isLoading ? <CircularProgress size={24} /> : <LoginIcon sx={{ fontSize: { xs: 22, md: 20 } }} />}
                    disabled={isLoading}
                  >
