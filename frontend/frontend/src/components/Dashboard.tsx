...
const Dashboard: React.FC = () => {
  ...
  const [form, setForm] = useState({ project: '', hours: '', description: '', start: null as Dayjs | null, end: null as Dayjs | null });
  const [filter, setFilter] = useState({ start: null as Dayjs | null, end: null as Dayjs | null });
  ...
  const handleAdd = () => {
    setEntries([
      { id: Date.now(), project: form.project, hours: form.hours, description: form.description, start: form.start?.format('DD/MM/YY') || '', end: form.end?.format('DD/MM/YY') || '', updated: dayjs().format('DD/MM/YY - HH:mm') },
      ...entries,
    ]);
    setOpen(false);
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex' }}>
      <LeftSidePanel />
      <Box sx={{ width: { xs: '100%', md: '50%' }, display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <Box sx={{ display: 'flex', gap: 2 }}>
                    <DatePicker label="Start Date" value={form.start} onChange={(date: Dayjs | null) => setForm(f => ({ ...f, start: date }))} slotProps={{ textField: { fullWidth: true, required: true } }} />
                    <DatePicker label="End Date" value={form.end} onChange={(date: Dayjs | null) => setForm(f => ({ ...f, end: date }))} slotProps={{ textField: { fullWidth: true, required: true } }} />
                  </Box>
            ...
            <DialogContent>
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                  <DatePicker label="Start Date" value={filter.start} onChange={(date: Dayjs | null) => setFilter(f => ({ ...f, start: date }))} slotProps={{ textField: { fullWidth: true } }} />
                  <DatePicker label="End Date" value={filter.end} onChange={(date: Dayjs | null) => setFilter(f => ({ ...f, end: date }))} slotProps={{ textField: { fullWidth: true } }} />
                </Box>
              </LocalizationProvider>
            </DialogContent>
            <DialogActions>
